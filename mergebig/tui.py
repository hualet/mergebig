...
from .scanner import FileInfo, format_size, scan_large_files, find_duplicates_by_hash, find_duplicates_by_name
...
class TUI:
    ...
    def _scan(self):
        ...
        self.message = f'扫描完成，找到 {len(self.files)} 个大文件'
    ...
    def _execute(self):
        mode = self.MODES[self.mode_idx][0]
        items = self._get_flat_items()
        selected_files = [f for t, f in items if t == 'file' and str(f.path) in self.selected]
        ...
        if mode == 'large':
            # 删除确认
            self.message = f'确认删除 {len(selected_files)} 个文件? 按 Enter 确认，其他键取消'
            self.message_color = 4
            self._draw()
            ...
            for group in self.groups:
                group_paths = [str(f.path) for f in group]
        ...
        for row in range(list_height):
            ...
            if item_type == 'sep':
                ...
                if len(path_str) > max_path_len:
                    path_str = '...' + path_str[-(max_path_len - 3):] if max_path_len > 3 else path_str[:max_path_len]
